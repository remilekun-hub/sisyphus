...
export default function Chart() {
  ...
  const symbol = `${slicedCurrentMarketPair[0]}${slicedCurrentMarketPair[1].toUpperCase()}`;
  ...
  const parsedVolume: HistogramData[] =
    chartData?.map((candle) => ({
      ...
      color: parseFloat(candle[4]) > parseFloat(candle[1]) ? "#4bffb5" : "#ff4976",
    })) ?? [];
  ...
  useEffect(() => {
    ...
    chart.current = createChart(chartContainerRef.current, {
      ...
      crosshair: { mode: window.innerWidth < 768 ? CrosshairMode.Magnet : CrosshairMode.Normal },
      grid: {
      ...
      timeScale: {
				...
				minBarSpacing: selectedRange.toLowerCase() === "1h" ? 5 : selectedRange.toLowerCase() === "2h" ? 10 : 15, 
			},
      rightPriceScale: {
        autoScale: true,
        entireTextOnly: true,
    ...
    const candleSeries = chart.current.addSeries(CandlestickSeries, {
      ...
      wickUpColor: "#838ca1",
      priceFormat: { type: "price", precision: window.innerWidth < 768 ? 2 : 0, minMove: 0.01 },
    });
      ...
      <div className="time-wrapper">
        ...
        <div className="time-buttons-container">
          {timeRange.map((time) => (
            <button
              key={time}
              className={`time-button ${selectedRange === time ? "active" : "inactive"}`}
